
  <?php wp_footer(); ?>
  <footer class="py-3 bg-dark mt-5 ">

<?php $lang=get_bloginfo("language"); 
    if($lang == 'es') { 
     echo "<image style='float: right;' class='mr-2'  src='data:image/png;base64,'></image>";
    }else{ 
     echo "<image style='float: right;' class='mr-2' src='data:image/png;base64,'></image>";
    }
 ?>

      <p class="m-0 mr-3 text-center text-white">Copyright &copy; <?php bloginfo('Sudo Su'); echo ' 2020 - '.date('Y'); ?> </p>

      <span id="current_language" hidden><?php echo pll_current_language(); ?></span>

  </footer>
</body>
      <!-- Si has leído el código hasta aquí, solo decirte que te quiero :-) , THE END... (?) -->
</html>
